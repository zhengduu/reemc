...
/* Changes by Zheng Du 25th October 2018 (zheng.du@tum.de)
 * - remove set pose function so tf skeleton is not fixed in rviz and moves accordingly
 *   to model in Axis Neuron software
 */
...
class NeuronBroadcaster {
 ...
 void sendStaticTransform() {
   ...
   // match orientation of REEM-C in simulation
   ...
   tf_broadcaster_.sendTransform(tf::StampedTransform(
       world_frame, ros::Time(0), "world", "WorldPerceptionNeuron"));
 }
 ...
 void callback(const std_msgs::Float32MultiArrayConstPtr& bone_data) {
   ...
   for (uint joint_index = 0; joint_index < bone_data->data.size() / 6;
     ...
     // not setting pose so tf skeleton in rviz is not static anymore
     /*
      * (uncomment for ur10)
     if (joint_index == 0) {
